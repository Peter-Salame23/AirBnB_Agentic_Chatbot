...
def generate_placeholder_image(listing: Dict, width: int, height: int) -> str:
    ...
    try:
        # You can drop a .ttf in assets/fonts/MyFont.ttf to customize
...
def image_for_listing(listing: Dict, width: int = 640, height: int = 400) -> str:
    """
    Priority:
      1) Local image in assets/images/<property_type>/
      2) Generated placeholder (offline, stable)
    """
    ...
    # Guaranteed fallback: offline generated image
    return generate_placeholder_image(listing, width, height)
    ...
    for i, r in enumerate(recs):
        with cols[i % columns]:
            with st.container(border=True):
                ...
                # Always-works image (local or generated)
                img_src = image_for_listing(r, width=640, height=400)
                ...
                lid = int(r.get("listing_id"))
                if st.button(f"Book (ID {lid})", key=f"book_card_{lid}"):
                    selected_id = lid
